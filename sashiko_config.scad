...
plate_w  = 120;   // plate width (mm)
plate_h  = 120;   // plate height (mm)
plate_t  = 0.8;   // plate thickness (mm) — slots are cut through all of it
groove_w = 1.2;   // slot width (mm)
border   = 6;     // solid border around plate edge (mm)
chamfer  = 0.3;   // 45° bevel on the four outer top edges (mm; 0 = off, keep <= t/2)
